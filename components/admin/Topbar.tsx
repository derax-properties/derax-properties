...
export function Topbar({ fullName, role, notificationCount }: { fullName: string | null; role: string | null; notificationCount: number }) {
  ...
  return (
    <header className="flex items-center gap-4 border-b border-ink/10 bg-white/70 px-4 py-3 backdrop-blur sm:px-8">
      <form onSubmit={onSearch} className="flex flex-1 items-center gap-2 rounded-full border border-ink/10 bg-white px-4 py-2">
      ...
      <Link href="/admin/leads" className="focus-gold relative shrink-0 rounded-full p-2 text-ink/50 hover:bg-ink/5" title="Leads needing attention">
      ...
      <div className="flex shrink-0 items-center gap-2.5">
        <span className="crm-accent-bg flex h-9 w-9 items-center justify-center rounded-full text-xs font-bold text-white">{initials(fullName)}</span>
